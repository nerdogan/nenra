...
class Stok(QtGui.QDialog , Ui_Dialog6):
    ...
    def sloturunmaliyet(self):
        ...
        sql = """select a.hamkod,a.hamad,a.birim , ifnull( b.miktar1,0) as giriş ,ifnull(c.miktar2,0) as çıkış, (ifnull( b.miktar1,0)-ifnull(c.miktar2,0)) as fark from hammadde a  left join table1 b on a.hamkod=b.hamkod left join table2 c on a.hamkod=c.hhammaddeid where bolum="bira" order by a.hamkod; """
        ...
        for row1 in bul:
            ...
            if (15 * (bb + 1)) >= 760:
                ...
                c.drawString(550, 800 - (15 * (bb + 1)), str(toplam2))
                c.showPage()
                c.setFont("Verdana", 8)
                bb = 0
